...
from datasets import WHOI_few_shot
...
def main(args):
    ...
    if args.target_dataset == 'WHOI':
        transform = WHOI_few_shot.TransformDataset(args.image_size).get_composed_transform(aug=True)
        transform_test = WHOI_few_shot.TransformDataset(args.image_size).get_composed_transform(aug=False)
        dataset = WHOI_few_shot.SimpleDataset(transform, split=args.target_subset_split)
    else:
